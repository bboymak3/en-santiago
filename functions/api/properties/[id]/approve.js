// functions/api/properties/[id]/approve.js
...
export async function onRequestPost(context) {
  try {
    ...
    if (!payload || payload.role !== 'admin') {
      return new Response(JSON.stringify({ error: 'Solo administradores pueden aprobar propiedades' }), {
        status: 403, headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }

    await env.DB.prepare("UPDATE properties SET status = 'approved', updated_at = datetime('now') WHERE id = ?").bind(id).run();
